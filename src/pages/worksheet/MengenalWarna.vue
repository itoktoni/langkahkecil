<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar" style="background: linear-gradient(135deg, #E91E63, #FCE4EC);">
      ...
      <div class="ws-toolbar-title"><Icon icon="mdi:palette-outline" class="ws-toolbar-icon" /> Mengenal Warna</div>
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box title-box">
            <span class="ws-title"><Icon icon="mdi:palette-outline" class="ws-title-icon" /> Mengenal Warna</span>
          </div>
        ...
        <div class="ws-grid3">
          <div v-for="(c, i) in colors" :key="i" class="ws-color-cell">
            <div class="ws-color-box"></div>
            <div class="ws-color-label">{{ c.name }}</div>
          </div>
        </div>
    </div>
  </div>
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_MengenalWarna.pdf')
  } catch (e) {
    console.error(e)
...
.ws-grid3 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; flex: 1; align-content: start; }
.ws-color-cell { display: flex; flex-direction: column; align-items: center; gap: 10px; }
.ws-color-box { width: 100%; aspect-ratio: 1; border: 3px solid #ccc; border-radius: 12px; background: white; }
.ws-color-label { font-size: 18px; font-weight: 700; color: #333; }
