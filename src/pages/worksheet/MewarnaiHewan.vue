<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar" style="background: linear-gradient(135deg, #43A047, #E8F5E9);">
      <button class="ws-btn-close" @click="$emit('close')">✕</button>
      <div class="ws-toolbar-title"><Icon icon="mdi:cat" class="ws-toolbar-icon" /> Mewarnai Hewan</div>
      <div class="ws-toolbar-actions">
        <button class="ws-btn" style="color: #2E7D32;" @click="downloadPDF" :disabled="generating">
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box title-box">
            <span class="ws-title"><Icon icon="mdi:cat" class="ws-title-icon" /> Mewarnai Hewan</span>
          </div>
...
const items = [
  { name: 'Kucing', icon: 'mdi:cat' },
  { name: 'Anjing', icon: 'mdi:dog' },
  { name: 'Kelinci', icon: 'mdi:rabbit' },
  { name: 'Ikan', icon: 'mdi:fish' },
  { name: 'Burung', icon: 'mdi:bird' },
  { name: 'Kupu-kupu', icon: 'mdi:butterfly-outline' }
]
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_MewarnaiHewan.pdf')
  } catch (e) {
    console.error(e)
...
.ws-btn { background: white; color: #2E7D32; border: none; padding: 8px 16px; border-radius: 10px; font-weight: 700; font-size: 13px; cursor: pointer; }
