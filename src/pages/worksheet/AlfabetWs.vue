<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar" style="background: linear-gradient(135deg, #1565C0, #E3F2FD);">
      ...
      <div class="ws-toolbar-title"><Icon icon="mdi:alphabet-latin" class="ws-toolbar-icon" /> Huruf Kapital A-Z</div>
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box title-box">
            <span class="ws-title"><Icon icon="mdi:alphabet-latin" class="ws-title-icon" /> Huruf Kapital A-Z</span>
          </div>
        ...
        <div class="ws-grid4">
          <div v-for="letter in letters" :key="letter" class="ws-letter-cell">
            <div class="ws-letter-display">{{ letter }}</div>
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_AlfabetWs.pdf')
  } catch (e) {
    console.error(e)
...
.ws-grid4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; flex: 1; }
.ws-letter-cell { border: 2px solid #ccc; border-radius: 8px; display: flex; flex-direction: column; align-items: center; padding: 8px 6px; gap: 0; }
.ws-letter-display { font-size: 36px; font-weight: 900; color: #333; line-height: 1.2; user-select: none; }
.ws-practice-dashes { width: 100%; border-bottom: 1.5px dashed #ccc; margin-top: 8px; }
