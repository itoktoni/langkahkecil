<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar" style="background: linear-gradient(135deg, #FB8C00, #FFF3E0);">
      ...
      <div class="ws-toolbar-title"><Icon icon="mdi:pencil-outline" class="ws-toolbar-icon" /> Mengikuti Garis Angka</div>
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box title-box">
            <span class="ws-title"><Icon icon="mdi:pencil-outline" class="ws-title-icon" /> Mengikuti Garis Angka</span>
          </div>
        ...
        <div class="ws-grid4">
          <div v-for="num in numbers" :key="num" class="ws-cell">
            <span class="ws-guide-num">{{ num }}</span>
            <span class="ws-dotted-num">{{ num }}</span>
          </div>
        </div>
    </div>
  </div>
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_TracingAngka.pdf')
  } catch (e) {
    console.error(e)
...
.ws-guide-num { font-size: 48px; font-weight: 900; color: #ddd; line-height: 1; user-select: none; }
.ws-dotted-num { font-size: 36px; font-weight: 900; color: transparent; -webkit-text-stroke: 1.5px #aaa; paint-order: stroke fill; line-height: 1; user-select: none; margin-top: 4px; }
