<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar" style="background: linear-gradient(135deg, #FB8C00, #FFF3E0);">
      <button class="ws-btn-close" @click="$emit('close')">✕</button>
      <div class="ws-toolbar-title"><Icon icon="mdi:food-apple-outline" class="ws-toolbar-icon" /> Mewarnai Buah</div>
      <div class="ws-toolbar-actions">
        <button class="ws-btn" style="color: #E65100;" @click="downloadPDF" :disabled="generating">
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box title-box">
            <span class="ws-title"><Icon icon="mdi:food-apple-outline" class="ws-title-icon" /> Mewarnai Buah</span>
          </div>
        ...
        <div class="ws-grid3">
          <div v-for="(item, i) in items" :key="i" class="ws-item-cell">
            <div class="ws-item-emoji"><Icon :icon="item.icon" class="ws-item-icon" /></div>
...
const items = [
  { name: 'Apel', icon: 'mdi:food-apple-outline' },
  { name: 'Pisang', icon: 'mdi:fruit-cherries' },
  { name: 'Anggur', icon: 'mdi:fruit-grapes-outline' },
  { name: 'Semangka', icon: 'mdi:fruit-watermelon' },
  { name: 'Jeruk', icon: 'mdi:fruit-citrus' },
  { name: 'Stroberi', icon: 'mdi:fruit-cherries' }
]
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_MewarnaiBuah.pdf')
  } catch (e) {
    console.error(e)
...
.ws-btn { background: white; color: #E65100; border: none; padding: 8px 16px; border-radius: 10px; font-weight: 700; font-size: 13px; cursor: pointer; }
...
.ws-grid3 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; flex: 1; align-content: start; }
.ws-item-cell { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; border: 2px solid #ccc; border-radius: 12px; padding: 20px; min-height: 130px; }
.ws-item-emoji { font-size: 52px; line-height: 1; display: flex; align-items: center; justify-content: center; }
.ws-item-icon { width: 48px; height: 48px; }
.ws-item-name { font-size: 16px; font-weight: 700; color: #333; }
