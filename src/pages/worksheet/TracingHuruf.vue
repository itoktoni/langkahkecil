<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar" style="background: linear-gradient(135deg, #43A047, #E8F5E9);">
      ...
      <div class="ws-toolbar-title"><Icon icon="mdi:pencil-outline" class="ws-toolbar-icon" /> Mengikuti Garis Huruf</div>
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box title-box">
            <span class="ws-title"><Icon icon="mdi:pencil-outline" class="ws-title-icon" /> Mengikuti Garis Huruf</span>
          </div>
        ...
        <div class="ws-grid4">
          <div v-for="letter in letters" :key="letter" class="ws-cell">
            <span class="ws-guide-letter">{{ letter }}</span>
            <span class="ws-dotted-letter">{{ letter }}</span>
          </div>
        </div>
    </div>
  </div>
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_TracingHuruf.pdf')
  } catch (e) {
    console.error(e)
...
.ws-grid4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; flex: 1; }
.ws-cell { border: 2px solid #ccc; border-radius: 8px; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 8px; min-height: 120px; }
.ws-guide-letter { font-size: 48px; font-weight: 900; color: #ddd; line-height: 1; user-select: none; }
.ws-dotted-letter { font-size: 36px; font-weight: 900; color: transparent; -webkit-text-stroke: 1.5px #aaa; paint-order: stroke fill; line-height: 1; user-select: none; margin-top: 4px; }
