<template>
  <div class="ws-wrapper">
    <div class="ws-toolbar">
      ...
      <div class="ws-toolbar-title"><Icon icon="mdi:link-variant" class="ws-toolbar-icon" /> Pasangan Benda</div>
    ...
    <div ref="printArea" class="ws-print-area">
        <div class="ws-header">
          <div class="ws-header-box ws-title-box">
            <span class="ws-title">Pasangan Benda</span>
          </div>
        ...
        <div class="ws-instruction">Hubungkan benda dengan fungsinya!</div>
        <div class="ws-content">
          <div v-for="(item, i) in items" :key="i" class="ws-match-row">
            <div class="ws-match-box ws-match-left">{{ item.benda }}</div>
            <div class="ws-match-dots"></div>
            <div class="ws-match-box ws-match-right">{{ shuffledFungsi[i] }}</div>
          </div>
        </div>
    </div>
  </div>
...
const items = [
  { benda: 'Pensil', fungsi: 'Menulis' },
  { benda: 'Sendok', fungsi: 'Makan' },
  { benda: 'Sepatu', fungsi: 'Berjalan' },
  { benda: 'Payung', fungsi: 'Hujan' },
  { benda: 'Sikat gigi', fungsi: 'Gigi bersih' },
  { benda: 'Bantal', fungsi: 'Tidur' }
]

const shuffledFungsi = computed(() => {
  const arr = items.map(i => i.fungsi)
...
async function downloadPDF() {
  ...
  try {
    ...
    await downloadWorksheetPDF(printArea.value, 'Worksheet_PasanganBenda.pdf')
  } catch (e) {
    console.error(e)
...
.ws-instruction {
  font-size: 14px; font-style: italic; color: #6A1B9A;
  margin-bottom: 16px; text-align: center;
}
.ws-content { display: flex; flex-direction: column; gap: 16px; }
.ws-match-row { display: flex; align-items: center; gap: 0; }
.ws-match-box {
  padding: 12px 20px; border: 2.5px solid #6A1B9A;
  border-radius: 10px; font-size: 16px; font-weight: 700;
  color: #6A1B9A; min-width: 140px; text-align: center;
}
.ws-match-left { background: #F3E5F5; }
.ws-match-right { background: #FCE4EC; }
.ws-match-dots {
  flex: 1; border-bottom: 2px dotted #aaa;
